...
def generate_anwsers(question: str, count: int, prev_anwsers: dict[int, tuple[str, bool]], retry = False) -> list[str]:
    ...
    if count == 5:
        ...
        request += "Die richtige Antwort bitte zuerst\nIncludiere keine Hinweise wie z.B. 'falsch', 'richtig', 'Falsche Antwort', 'Richtige Antwort', 'Antwort' oder ähnliches.\nNutze keine Aufzählungszeichen wie z.B. 1. oder A)\nNutze | als Delimiter."
    else:
...
def sanitizeGPT(content: str)-> list[str]:
    splits = re.split(r"\||\n", content)
